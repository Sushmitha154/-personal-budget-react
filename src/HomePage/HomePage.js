...
const HomePage = () => {
    ...
    function getBudget() {
        axios.get('http://localhost:3000/budget')
            .then(function (res) {
                ...
                const newBudget = { ...budget }; // Create a copy of the state
                for (var i = 0; i < res.data.myBudget.length; i++) {
                    newBudget.datasets[0].data[i] = res.data.myBudget[i].budget;
                    newBudget.datasets[0].backgroundColor[i] = res.data.myBudget[i].color;
                    newBudget.labels[i] = res.data.myBudget[i].title;
                }
                setBudget(newBudget); // Use setBudget to update state
                createDoughnut();
            })
    ...
    // Function to create donut chart using D3.js
    const getChart = () => {
        axios.get('http://localhost:3000/example')
            .then(res => {
                console.log("Donut Chart Data:", res.data.myBudget);
    ...
    return (
        <main className="center" id="main">
            ...
            <article class="text-box">
                ...
                <h1>Drawing this using canvas</h1>
                <p><canvas id="myChart" width="800" height="300"></canvas></p>
                ...
                <svg id="chart-container" width="800" height="400"></svg>
            </div>
        </main>
    );
}
